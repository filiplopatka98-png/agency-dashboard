-- Audit 2026-07-17, 2.6: nevážený priemer uptime na verejnej status page.
-- `avg(uptime_pct)` v `uptime30` aj vo `vig` CTE počíta priemer DENNÝCH
-- percent — deň s 3 kontrolami tak váži rovnako ako deň s 288. Skutočné
-- číslo je SUM(up) / SUM(checks) za obdobie; `uptime_daily` už oba stĺpce
-- (`checks`, `up`) ukladá práve na tento účel. `hist` CTE (denná história pre
-- graf) sa NEMENÍ — tam je `ud.uptime_pct` hodnota JEDNÉHO dňa, nie agregát,
-- takže vážiť nie je čo.
--
-- `nullif(sum(checks), 0)` — ak by za dané okno neboli žiadne kontroly
-- (0 riadkov alebo checks=0 v každom), delenie nulou by inak vyhodilo chybu;
-- výsledok ostáva `null` rovnako ako predtým pri `avg()` nad prázdnou množinou.
