-- Audit 2026-07-17, item 3: `public_client_status` počítal dve 90-dňové okná
-- dvoma rôznymi spôsobmi — `hist`/`vig` (uptime_daily) filtrujú dátumovou
-- aritmetikou `ud.day >= (current_date - 90)`, zatiaľ čo `inc` (incidents)
-- filtroval timestamptz aritmetikou `i.started_at >= (now() - interval
-- '90 days')`. `current_date - 90` je polnoc (00:00) pred 90 dňami; `now() -
-- interval '90 days'` je AKTUÁLNY čas pred 90 dňami — až ~24h rozdiel podľa
-- toho, kedy počas dňa sa dopyt spustí. Dôsledok: incident tesne za hranicou
-- uptime histórie sa mohol zobraziť v "posledných 90 dňoch" incidentov, ale
-- chýbať v dennej histórii uptime (alebo naopak) — kozmetická nekonzistencia
-- na verejnej status stránke, ale zbytočná.
--
-- Fix: zjednotiť `inc` na rovnakú dátumovú aritmetiku ako `hist`/`vig`
-- (`current_date - 90`, cast na timestamptz o polnoci) — obe okná teraz majú
-- identickú hranicu.
...
language sql security definer set search_path = public stable as $$
  ...
  s as (
    select st.id, st.domain, st.maintenance, st.consecutive_failures,
           ...
           (select round((sum(ud.up)::numeric / nullif(sum(ud.checks), 0)) * 100, 2) from uptime_daily ud
  ...
  vig as (
    select ud.site_id, sum(ud.checks)::bigint as checks,
           round((sum(ud.up)::numeric / nullif(sum(ud.checks), 0)) * 100, 2) as uptime_pct
    from uptime_daily ud join s on s.id = ud.site_id
    where ud.day >= (current_date - 90)
    group by ud.site_id
  ),
  inc as (
    ...
    where i.resolved_at is not null and i.started_at >= (current_date - 90)
    group by i.site_id
  )
