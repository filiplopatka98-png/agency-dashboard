...
describe('runJobHealth — sixhourly job (asset-check) & GitHub runner výpadok (overdueFactor)', () => {
  const h = 3_600_000;
  it('asset-check beh spred 12 h → NIE je overdue (GitHub runner výpadok, nie mŕtvy)', async () => {
    const store = baseStore();
    store.job_runs.push({ job: 'asset-check', status: 'ok', finished_at: new Date(NOW.getTime() - 12 * h).toISOString() });
    await runJobHealth(env, { supabase: fakeSupabase(store), now: NOW });
    expect(jobOverdue(store)).toHaveLength(0);
  });
  it('asset-check beh spred 25 h → overdue (reálne mŕtvy; prah je 24 h)', async () => {
    const store = baseStore();
    store.job_runs.push({ job: 'asset-check', status: 'ok', finished_at: new Date(NOW.getTime() - 25 * h).toISOString() });
    await runJobHealth(env, { supabase: fakeSupabase(store), now: NOW });
